...
async function run() {
  try {
    ...
    const pendingOrderCollection = client
      .db("ShopDB")
      .collection("pendingOrders");
    const selectedOrderCollection = client
      .db("ShopDB")
    ...
    app.post("/addToCart", async (req, res) => {
      ...
      if (checkProduct) {
        ...
        const resultCart = await cartsCollection.updateOne(
          query,
          updateDoc,
          options
        );
    ...
    app.put("/orders/:orderId", async (req, res) => {
      ...
      const result = await pendingOrderCollection.updateOne(
        query,
        updateDoc,
        options
      );
    ...
    app.get("/all-orders", verifyToken, verifyAdmin, async (req, res) => {
      ...
      const result = await pendingOrderCollection
        .find(query)
        .sort({ _id: -1 })
        .toArray();
      res.send(result);
    ...
    app.get("/stockProductCount/:productIds", async (req, res) => {
      const ids = req.params.productIds
        .split(",")
    ...
    app.delete("/view-order-delete/:orderId/:index", async (req, res) => {
      ...
      try {
        ...
        const updateResult = await pendingOrderCollection.updateOne(query, {
    ...
    app.get("/confirmOrder", verifyToken, verifyAdmin, async (req, res) => {
      ...
      const result = await confirmOrderCollection
        .find(query)
        .sort({ _id: -1 })
        .toArray();
      res.send(result);
